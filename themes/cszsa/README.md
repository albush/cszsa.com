...
Dimension is a single page and responsive site template. It is a port of [HTML5 UP's Dimension theme](//html5up.net/uploads/demos/dimension/).

![Dimension Theme screenshot](//raw.githubusercontent.com/sethmacleod/dimension/master/images/screenshot.png)

## Installation

Run the following commands inside your Hugo site folder:

    $ git clone //github.com/sethmacleod/dimension.git themes/dimension
...
For this run the following commands inside your Hugo site folder:
    ...
    $ git submodule add //github.com/sethmacleod/dimension.git themes/dimension
...
Copy the `config.toml` from the exampleSite folder into your Hugo site's root folder. Change the fields as needed. Add or delete social media by following the examples in the file. You may need to look up the [font-awesome](//fontawesome.io/) icon names. The icon field should be filled out without the "fa" prefix. The icon field for Twitter should be 'twitter' instead of 'fa-twitter'.
...
You will need to use an external service for the contact form since static sites cannot handle forms on their own. One such service is [Formspree](//formspree.io/). Formspree has a free tier. Check out the `formspree.md` page in the exampleSite folder for a template.

### Tabs as Links

You may want a tab to link to external content. Create your page, and set a variable `link` in the front matter to your desired output. For example: `link = "//www.example.org"`. Note: this method could be used to link to internal pages, but the theme does not support this out of the box.
...
## Google Analytics
To include Google Analytics, you can follow the standard Hugo way and just add `googleAnalytics = "UA-123-45"` to the `config.toml` as described on [Analytics in Hugo](//gohugo.io/extras/analytics/). 
...
This theme is released under the CC BY 3.0 license. For more information, read the [License](//github.com/sethmacleod/dimension/blob/master/LICENSE.md).
